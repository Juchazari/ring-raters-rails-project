...
@user_02 = User.create(first_name: "Francisco", last_name: "Sierra", email: "fransierra.s.15@gmail.com", password: "franciscoonion")
...
@review_01 = Review.create(description:"Spencer's Holly Onions has a brilliant atmosphere for a good time out with friends. The color pallet is rustic greens, and browns, and the seating near the windows was extremely comfortable. Unfortunately, their secret sauce onion rings didnt quite live up to the hype. The rings were too oily, and the fabled sauce had an odd pungent smell. I'll give it three stars for a great interior, and some mediocre onion rings" , rating: 3 , onion_ring_id: @onion19.id, user_id: @user_09.id)
@review_02 = Review.create(description: "the wrong type of rings. This was a huge disappointment. They were wayyyyy too salty, and when I asked the server for a new batch they spit in my eye. Dont know what a Kolach is but im never coming here again", rating: 1, onion_ring_id: @onion22.id, user_id: @user_11.id)
@review_03 = Review.create(description: "Orange Onions! A novel concept! The sweetness of the oranges in the sauce, and the zest in the batter paired well with the spices, and the flavour of the onion, Antonio's Onion Kitchen hit this recipe out of the park!" , rating: 4, onion_ring_id: @onion3.id, user_id: @user_10.id)
@review_04 = Review.create(description: "The Crunchy Bluw Onions certainly live up its name. The rings were extremely crunchy, and cooked to perfection. The blue cheese sauce that was paired with it elevated this dish. The Hotel/Bar was also an interesting find, and one that I'll be visiting later for sure!", rating: 5 , onion_ring_id: @onion7.id, user_id: @user_09.id)
@review_05 = Review.create(description: "It reminded me of my grandma's seasoning. She was such a lovely woman, just like this lovely onion. I loved it!", rating: 5, onion_ring_id: @onion1.id, user_id: @user_02.id)
@review_06 = Review.create(description: "I mean, it tasted good, but what's the point anyway? We are all going to die", rating: 1, onion_ring_id: @onion2.id, user_id: @user_02.id)
@review_07 = Review.create(description: "I ate here and ended up in the ICU. Enough said", rating: 1, onion_ring_id: @onion3.id, user_id: @user_02.id)
@review_08 = Review.create(description: "This is a bad place to go if you are trying to lose weight. Their food has flavor and makes you want to wait it all", rating: 5, onion_ring_id: @onion4.id, user_id: @user_02.id)
@review_09 = Review.create(description: "You might as well just shit on an onion and mix it some sauce... you wouldn't know the difference", rating: 1, onion_ring_id: @onion5.id, user_id: @user_02.id)
@review_10 = Review.create(description: "They charged me $150 for a plate of 15 onion rings. What the heck?", rating: 1, onion_ring_id: @onion6.id, user_id: @user_02.id)
@review_11 = Review.create(description: "I asked if they had meatballs and the waiter yelled YOU CAN MEET THESE BALLS! Then he flipped the table and walked out. 1 star because I never got to meet those balls... this place doesn't deliver on what they offer", rating: 1, onion_ring_id: @onion8.id, user_id: @user_02.id)
@review_12 = Review.create(description: "The worst thing since herpes", rating: 1, onion_ring_id: @onion9.id, user_id: @user_02.id)
@review_13 = Review.create(description: "The meal was delectable, exotic, and incinerated everything in my intestines!", rating: 5, onion_ring_id: @onion10.id, user_id: @user_02.id)
@review_14 = Review.create(description: "The restaurant space is very clean and nice, with a great casual vibe. The food has been outstanding. I love the burger and especially the tater tots. The burger has a unique smoky flavor to it. Outdoor patio is nice as well. I think they hit the nail on the head.", rating: 5, onion_ring_id: @onion11.id, user_id: @user_02.id)
@review_15 = Review.create(description: "Stars only because location is nice. Food is bland sysco and worse than any fast food establishment. Burger patty is a tasteless piece of thin meat with a bun that is small, hard and most likely cheapest bun you can get. Fries and tots are fried in bigger batches so by the time you get your order...your side of fries and tots are cold!", rating: 2, onion_ring_id: @onion12.id, user_id: @user_02.id)
@review_16 = Review.create(description: "Overall the food is good.  I ordered the burger with grill onions and it was pretty tasty.   I also brought home a chicken bowl which was also good. The prices were decent and the location was clean. I", rating: 5, onion_ring_id: @onion13.id, user_id: @user_02.id)
@review_17 = Review.create(description: "Great service, great onions!", rating: 5, onion_ring_id: @onion14.id, user_id: @user_02.id)
